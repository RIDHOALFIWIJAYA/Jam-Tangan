...
import { getDetailUrl, imgFallback, formatCurrency } from "../data/api";
    ...
    <div className="glass p-4 rounded-[35px] hover:bg-blue-600/5 transition-all duration-700 group hover:-translate-y-3 flex flex-col h-full relative">
      ...
      <div>
        <div className="relative overflow-hidden rounded-[25px] mb-6">
          ...
          <div className="absolute top-4 left-4 right-4 flex flex-wrap gap-1.5 pointer-events-none">
            {product.kategori.map((kat) => (
              <div
                key={kat}
                className="bg-black/60 backdrop-blur-md text-[8px] text-white px-2 py-0.5 rounded-full uppercase tracking-[0.2em] font-black border border-white/10"
              >
                {kat}
              </div>
            ))}
        ...
        <div className="px-2">
          ...
          <p className="text-blue-400 mb-6 font-black text-lg italic">{formatCurrency(product.harga)}</p>
        </div>
      </div>
